...
import { RunClearPartitionsJob } from '../../src/application/use-cases/RunClearPartitionsJob.js';
...
test('clears each partition (mapped ids then orphan sweep), wipes ops, and does NOT re-sync', async () => {
  ...
  const uc = new RunClearPartitionsJob(deps);
  ...
  // checkpoint after the partition — under this job's own scope
  assert.deepEqual(calls.setState.find((s) => s.scope === 'clear_partitions:2026').patch.checkpoint, 1);
  // ops wipe
  assert.deepEqual(calls.clearYearMappings[0], { year: 2026 });
  assert.deepEqual(calls.resetPartitions[0], { year: 2026 });
  assert.ok(calls.clearState.some((c) => c.scope === 'full_sync:2026')); // future backfill starts clean
  // NO rebuild — nothing re-inserted
  assert.equal(calls.enqueueFull.length, 0);
  // finish + complete (secret scrubbed)
  assert.ok(calls.clearState.some((c) => c.scope === 'clear_partitions:2026'));
